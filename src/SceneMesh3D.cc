...
float find_mena_var_colour(cv::Mat image_aa, cv::Point2f startp, cv::Point2f endp, cv::Vec3f& mean1, cv::Vec3f& mean2, cv::Mat orientation) {
    ...
    // Draw shifted lines
    line(black_imag, startp_shifted1, endp_shifted1, cv::Scalar(0, 0, 255), paxos, 8);
    line(black_imag, startp_shifted2, endp_shifted2, cv::Scalar(255, 0, 255), paxos, 8);
    ...
    for (int k = 0; k < black_imag.cols; k++) {
        for (int l = 0; l < black_imag.rows; l++) {
            ...
            if (color2 == cv::Vec3b(0, 0, 255) && orientation.at<double>(l, k) != 228360) {
                cv::Vec3b color3 = image_aa.at<cv::Vec3b>(l, k);
                mean1 += cv::Vec3f(color3[0], color3[1], color3[2]);
                arithmos_pixel1++;
            } else if (color2 == cv::Vec3b(255, 0, 255) && orientation.at<double>(l, k) != 228360) {
    ...
    if (arithmos_pixel1 > 0) mean1 /= arithmos_pixel1;
    if (arithmos_pixel2 > 0) mean2 /= arithmos_pixel2;
    ...
    for (int k = 0; k < black_imag.cols; k++) {
        for (int l = 0; l < black_imag.rows; l++) {
            ...
            if (color2 == cv::Vec3b(0, 0, 255) && orientation.at<double>(l, k) != 228360) {
                cv::Vec3b color3 = image_aa.at<cv::Vec3b>(l, k);
                var1 += cv::Vec3f(pow((color3[0] - mean1[0]), 2), pow((color3[1] - mean1[1]), 2), pow((color3[2] - mean1[2]), 2));
                arithmos_pixel1++;
            } else if (color2 == cv::Vec3b(255, 0, 255) && orientation.at<double>(l, k) != 228360) {
                cv::Vec3b color4 = image_aa.at<cv::Vec3b>(l, k);
                var2 += cv::Vec3f(pow((color4[0] - mean2[0]), 2), pow((color4[1] - mean2[1]), 2), pow((color4[2] - mean2[2]), 2));
                arithmos_pixel2++;
            }
        }
    }

    if (arithmos_pixel1 > 0) var1 /= arithmos_pixel1;
    if (arithmos_pixel2 > 0) var2 /= arithmos_pixel2;
...
float find_ocr_space(cv::Mat image_aa, cv::Point2f startp, cv::Mat orientation, cv::Point2f endp) {
    ...
    float diafora_xr = find_mena_var_colour(image_aa, startp, simeio_tomis, mean_1, mean_2, orientation);
    ...
    for (double i = 0; i < 1; i += bima) {
        ...
        if (!(k < 0 || l < 0 || l >= orientation.cols || k >= orientation.rows) && orientation.at<double>(k, l) != 228360) {
            ...
            double dot_product = (line_vec[0] * temp_vec[0] + line_vec[1] * temp_vec[1]) /
...
void recalculate_v_p(cv::Point v_p, const cv::Mat& texture_orientation_a, const cv::Mat& image_g_f, const std::string& name, cv::Mat& result) {
    ...
    int max_col = v_p.x; // Using v_p.x instead of v_p.y, assuming v_p is a (x,y) point
    int max_row = v_p.y;
    ...
    for (int i = 15; i < 170; i += 5) {
        ...
        int end_x = max_col - ((image_clone.rows - max_row) / sin_angle) * cos_angle;
        int end_y = max_row + ((image_clone.rows - max_row) / sin_angle) * sin_angle;
    ...
    for (size_t i = 1; i < rays.size() - 1; ++i) {
        ...
        float ocr_ratio = find_ocr_space(image_clone, cv::Point(max_col, max_row), texture_orientation, rays[i].second);
        ...
        disconnection_points.back().second = 0;  // Placeholder for xanei_epafi (disconnection)
    }
    ...
    cv::arrowedLine(image_clone, cv::Point(0, disconnect_point), cv::Point(image_clone.cols, disconnect_point), cv::Scalar(0, 220, 0), 2, 8);
    cv::Point mid_point = best_ray_start + 0.5 * (best_ray_end - best_ray_start);
    cv::arrowedLine(image_clone, cv::Point(max_col, max_row), mid_point, cv::Scalar(255, 255, 125), 2, 8);
    cv::arrowedLine(image_clone, cv::Point(max_col, max_row), mid_point, cv::Scalar(255, 0, 125), 2, 8);

    // Draw the point at the center
    cv::circle(image_clone, cv::Point(max_col, max_row), 5, cv::Scalar(0, 255, 0), cv::FILLED, 8);
